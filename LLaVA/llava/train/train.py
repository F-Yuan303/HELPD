...
#     rank0_print("Initilizing reward model...")
#     start = time.time()
#     reward_model_dir = "/data/share/LLMs/llama2-7b-chat-hf"
#     cuda_list = '0,1,2,4'.split(',')
#     memory = '24GiB'
#     max_memory = {int(cuda):memory for cuda in cuda_list}
#     no_split_module_classes = LlamaForCausalLM._no_split_modules
#     config = LlamaConfig.from_pretrained(reward_model_dir)

#     with init_empty_weights():
#         reward_model = LlamaForCausalLM._from_config(config, torch_dtype=torch.float16)
#     reward_model.tie_weights()
#     device_map = infer_auto_device_map(reward_model, max_memory=max_memory,no_split_module_classes=no_split_module_classes)

#     load_checkpoint_in_model(reward_model,reward_model_dir,device_map=device_map) #加载权重
#     model = dispatch_model(reward_model,device_map=device_map)
    
#     # reward_model = LlamaForCausalLM.from_pretrained(reward_model_dir, device_map='auto')
...
#     #     reward_model.get_input_embeddings().register_forward_hook(make_inputs_require_grad)
#     reward_model.eval()
#     end = time.time()
#     rank0_print(f"Done... Spend:{end-start}")
#     prompt_1="""
# You will be presented with two pieces of text that describe the same image. First text comes from a dataset and can be considered as the label, and the other is generated by a model. Your task is to compare these two pieces of text and evaluate them from the perspective of hallucination. If the contents described by the two pieces of text are completely consistent and there is no hallucination, please give a score of 0. If hallucination is present in the model-generated text, i.e., it describes content not present in the dataset text, please give a score closer to 1. Note that the lower the score, the less obvious the hallucination. The score range is between 0 and 1. Please only provide a score and do not provide reason.
...
# Dataset text (Label): "In the image, a young woman with short blond hair is wearing jeans and a striped long-sleeved sweater as she jumps in midair on a skateboard, with trees in the background. Nearby, a young man is also in midair while skateboarding at a skate park, with people watching him in the background. It appears that he is performing a trick in a skateboarding competition, as a crowd of onlookers observes the impressive feat. Additionally, a boy can be seen jumping on his skateboard with a crowd watching him. Overall, the scene captures someone skateboarding at a skate park, with various individuals taking in the exhilarating display."
# Model generated text: "The image shows a young boy performing tricks on his skateboard in front of an audience at the park. He is wearing white pants and black shoes, with blonde hair tied back into two braids behind him as he jumps off one end of the ramp onto another below it while landing gracefully afterward"
# score:
# """
#     inputs = reward_tokenizer(prompt_1, return_tensors="pt").to(reward_model.device)
#     input_length = inputs.input_ids.shape[1]
#     # Generate
#     generate_ids = reward_model.generate(inputs.input_ids, max_length=2048)['sequences']
#     text = reward_tokenizer.batch_decode(generate_ids[:,input_length:], skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
#     end = time.time()
#     text = text.split('score:')
#     print(text[-1])
    ...
    # print(f"training_args.gradient_checkpointing:{training_args.gradient_checkpointing}")
